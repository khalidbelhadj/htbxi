...
class TomTom:
    ...
    def __init__(self, place_name="Greater London, UK", mode: Literal['drive', 'walk', 'bike']='walk', speed: Optional[float]=None, mock = False):
        ...
        cache_file = f"{cache_path}/{place_name.replace(' ', '').lower()}-{mode}.graphml"
        try:
            if os.path.exists(cache_file):
                ...
                self.G = ox.graph_from_place(place_name, network_type=mode, simplify=True)
                print("Saving to cache...")
                ox.save_graphml(self.G, cache_file)
            
            # Prepare nodes for routing
            self.nodes = ox.graph_to_gdfs(self.G, edges=False)
            node_coords = np.array([[node['y'], node['x']] for _, node in self.nodes.iterrows()])
    ...
    def calculate_route(self, start, end):
        ...
        if route is None:
            raise PathFindingError(f"Failed on {self.mode} with start={start} and end={end}")
        return route

    def calculate_route_time(self, start, end):
        route = self.calculate_route(start, end)   
        length = sum(self.G.edges[route[i], route[i + 1], 0]['length'] for i in range(len(route) - 1))
        if length==0:
    ...
    def filter_districts_within_time(self, workplace_district, districts, max_travel_time):
        filtered_districts = {}
        for district in districts:
            travel_time = self.calculate_route_time(Point(districts[district]['latitude'], districts[district]['longitude']), Point(districts[workplace_district]['latitude'], districts[workplace_district]['longitude']))
