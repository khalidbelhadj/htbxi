...
def get_district_names():
    ...
    district_data = raw_data.parse(
        "3",
        skiprows=2,
        names=["District", "Category", "Count", "Mean", "LowerQ", "Median", "UpperQ"],
    )
...
def get_rent_data(file_path="data/rent_data.xlsx"):
    ...
    district_data["Mean"] = district_data[district_data["Mean"].apply(is_numeric)][
        "Mean"
    ]
    district_data["LowerQ"] = district_data[district_data["LowerQ"].apply(is_numeric)][
        "LowerQ"
    ]
    district_data["Median"] = district_data[district_data["Median"].apply(is_numeric)][
        "Median"
    ]
    district_data["UpperQ"] = district_data[district_data["UpperQ"].apply(is_numeric)][
        "UpperQ"
    ]
    burrough_data = raw_data.parse(
        "2",
        skiprows=2,
        names=["Burrough", "Category", "Count", "Mean", "LowerQ", "Median", "UpperQ"],
    )
    burrough_data["Mean"] = burrough_data[burrough_data["Mean"].apply(is_numeric)][
        "Mean"
    ]
    burrough_data["LowerQ"] = burrough_data[burrough_data["LowerQ"].apply(is_numeric)][
        "LowerQ"
    ]
    burrough_data["Median"] = burrough_data[burrough_data["Median"].apply(is_numeric)][
        "Median"
    ]
    burrough_data["UpperQ"] = burrough_data[burrough_data["UpperQ"].apply(is_numeric)][
        "UpperQ"
    ]
    # only take where category is 'One Bedroom' or 'Studio' and merge the categories
    district_data = district_data[
        district_data["Category"].isin(["One Bedroom", "Studio"])
    ]
    burrough_data = burrough_data[
        burrough_data["Category"].isin(["One Bedroom", "Studio"])
    ]
    district_data["Category"] = district_data["Category"].apply(
        lambda x: "One Bedroom" if x == "1 Bedroom" else "Studio"
    )
    burrough_data["Category"] = burrough_data["Category"].apply(
        lambda x: "One Bedroom" if x == "1 Bedroom" else "Studio"
    )
    # merge the categories
    district_data = (
        district_data.groupby(["District", "Category"])
        .agg({"Mean": "mean", "LowerQ": "mean", "Median": "mean", "UpperQ": "mean"})
        .reset_index()
    )
    burrough_data = (
        burrough_data.groupby(["Burrough", "Category"])
        .agg({"Mean": "mean", "LowerQ": "mean", "Median": "mean", "UpperQ": "mean"})
        .reset_index()
    )
