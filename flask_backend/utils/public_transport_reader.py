...
def filter_districts_by_distance(
    workplace_district,
    workplace_latitude,
    workplace_longitude,
    districts,
    ...
    filtered_districts_dict = {
        k: v for k, v in districts.items() if k in nearest_districts
    }
    ...
    if travel_cache is None:
        logging.info(
            f"No travel cache provided, calculating distances on-the-fly for workplace district: {workplace_district}"
        )
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for district, journey_duration in executor.map(
                get_journey_with_rate_limit, district_calculations
            ):
...
def get_all_distances(districts):
    ...
    for i, district1 in enumerate(district_names):
        for district2 in district_names[
            i + 1 :
        ]:  # Only consider districts that come after district1
            district_pairs.append((district1, district2))

    logging.info(
        f"Will calculate {len(district_pairs)} one-way journeys between districts"
    )
    ...
    def get_journey_with_rate_limit(pair):
        ...
        try:
            ...
            logging.error(
                f"Error calculating journey from {district1} to {district2}: {str(e)}"
            )
    ...
    completed = 0
    with ThreadPoolExecutor(max_workers=10) as executor:
        for district1, district2, journey_duration in executor.map(
            get_journey_with_rate_limit, district_pairs
        ):
            if journey_duration is not None:
                ...
                distances[(district2, district1)] = (
                    journey_duration  # Assume return journey takes the same time
                )

                completed += 1
                if completed % 10 == 0:
                    logging.info(
                        f"Completed: {completed}/{len(district_pairs)} ({completed/len(district_pairs)*100:.2f}%)"
                    )
...
def get_journey(from_lat, from_lon, to_lat, to_lon):
    ...
    if r.status_code != 200:
        raise Exception(
            f"Failed to get journey: from {from_lat},{from_lon} to {to_lat},{to_lon} with status code {r.status_code}"
        )

    journeys = r.json()["journeys"]
    if len(journeys) < 1:
        raise Exception(
            f"No journey found: from {from_lat},{from_lon} to {to_lat},{to_lon}"
        )
...
if __name__ == "__main__":
    ...
    print(
        filter_districts_by_distance(
            "BR1", 51.41204637320559, 0.0209232400318979, districts, 30
        )
    )
